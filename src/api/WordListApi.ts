...
export class WordListApi extends APIClient {
  //検索
  public search(
    request: WordListSearchRequest
  ): Promise<WordListResponse | null> {
    const url = '/wordList/search';
    const path = this.combineUrl(url);

    return this.httpPost<WordListSearchRequest, WordListResponse>(
      path,
      request
    );
  }
  ...
  //追加・更新
  public save(request: WordListRequest): Promise<WordListSaveResponse | null> {
  ...
  public delete(
    ...
    return this.httpPost<WordListRequest, WordListDeleteResponse>(
      path,
      request
    );
  }
 ...
 *interfaces
 */
export interface WordListSearchRequest {
  text: string | null;
}
